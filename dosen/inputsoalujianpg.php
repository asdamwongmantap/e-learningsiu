               ...
               <h6>Pilih Soal Ujian</h6>
                <div class="row">
                  <div class="span7 content-wrapper clearfix">
                    <div class="right-col">
                      <form action="simpansoalujianpgdetail.php" enctype="multipart/form-data" method="post">
                       <ul class="rr">
                        <table  style="border: 0pt dashed #0000CC;float:left;width:100%;" border="0" >
						<?php
						$idsoalujian=$_GET['id_soalujian'];
						?>
						<tr><td><input class="text-input"  style="width:50%;" type="text" name="id_soalujian" placeholder="ID Soal" value="<?php echo $idsoalujian;?>"/></td></tr>
                          <tr><td><select name='id_soal' id='s2_1' style='width: 40%;' onchange="changeValue(this.value)">
								  <option value=0 selected>- Pilih Soal -</option>
								  <?php
										$tampil=mysql_query("SELECT * FROM tbl_banksoalpg ORDER BY kd_mk");
										$jsArray = "var jrs = new Array();\n"; 
										while($r=mysql_fetch_array($tampil))
{
										  echo "<option value=$r[id_soal]>$r[id_soal]</option>";
										  $jsArray .= "jrs['" . $r['id_soal'] . "'] = {namajrs:'" . addslashes($r['soal']) . "',pilhana:'" . addslashes($r['pil_a']) . "', pilhanb:'" . addslashes($r['pil_b']) . "', pilhanc:'" . addslashes($r['pil_c']) . "', pilhand:'" . addslashes($r['pil_d']) . "'};\n";  
						  ...
						  <tr><td>A.<input class="text-input" style="color:black;" name="soal" id="pilhana" disabled></td></tr>
						  <tr><td>B.<input class="text-input" style="color:black;" name="soal" id="pilhanb" disabled></td></tr>
						  <tr><td>C.<input class="text-input" style="color:black;" name="soal" id="pilhanc" disabled></td></tr>
						  <tr><td>D.<input class="text-input" style="color:black;" name="soal" id="pilhand" disabled>
