...
from tbl_pengguna p, tbl_dosen d where p.username=d.nid AND p.username='$nid'");
  ...
  box-shadow: 1px 1px 2px rgba(0,0,0,.3);"><center>
            ...
            <ol class="rr">
              <li class="current" >
               <h6>Preview Soal Ujian</h6>
                <div class="row">
                  <div class="span7 content-wrapper clearfix">
                    <div class="right-col">
					<?php
						$idsoalujian=$_GET['id_soalujian'];					
						$edit = mysql_query("SELECT * FROM tbl_soal WHERE id_soalujian='$idsoalujian'");
						$r    = mysql_fetch_array($edit);
						?>
                      <form action="?hal=editujianpg&id=<?php echo $r['id_soalujian']?>" enctype="multipart/form-data" method="post">
                       <ul class="rr">
                        <table  style="border: 0pt dashed #0000CC;float:left;width:100%;" border="0" >
                          <tr><td><input class="text-input"  style="width:50%;" type="text" disabled name="id_soalujian" placeholder="ID Soal Ujian" value="<?php echo $idsoalujian ?>"/></td></tr>
                         <tr><td><select  disabled name='kdmk' id='s2_1' style='width: 100%;'>
						  <?php
                          ...
                          <tr><td><select  disabled name='kdkelas' id='s2_1' style='width: 100%;'>
						  <?php
						...
						?>
						   ...
						   <tr><td><input disabled class="text-input"  style="width:50%;" type="text" name="nid" value=<?php echo $r['nid']; ?> placeholder="NID"/></td></tr>
						   <tr><td><input disabled class="text-input"  style="width:50%;" type="text" name="waktu"  value=<?php echo $r['waktu']; ?> placeholder="Waktu Ujian"/></td></tr>
						   <tr><td>
						   <select disabled name='tipe' id='s2_1' style='width: 100%;'>
						  <?php
								  $tampil=mysql_query("SELECT * FROM tbl_soal ORDER BY id_soalujian");
								  if ($r[tipe]==0){
									echo "<option value=0 selected>- Pilih Tipe Ujian -</option>";
								  }   
								  
                                  while($w=mysql_fetch_array($tampil)){
									if ($r[tipe]==$w[tipe]){
									  echo "<option value=$w[tipe] selected>$w[tipe]</option>";
									}
									else{
									  echo "<option value=$w[tipe]>$w[tipe]</option>";
									}
								  }  
						?>
                       </select>
								  </td></tr>
							<tr><td><input class="text-input"  disabled style="width:50%;" type="date" name="tgl_ujian"  value=<?php echo $r['tgl_ujian']; ?> /></td></tr>
                          
                           <tr><td><input type="submit" class="btn secondary" value="EDIT"></td></tr>
...
<table class="layout display responsive-table">
    ...
    <thead>
	<tr>
	<td colspan="3"><a class="btn" href="?hal=inputsoalujianpg&id_soalujian=<?php echo $r['id_soalujian'];?>">TAMBAH SOAL</a>
	</tr>
        <tr>
			<th style="width:10%;">ID SOAL</th>                      
    ...
    <tbody>
	<?php
		$tp=mysql_query("SELECT tbl_soaldetail.id_soalujian, tbl_soaldetail.id_soal, tbl_banksoalpg.soal 
FROM tbl_soaldetail, tbl_banksoalpg
WHERE tbl_soaldetail.id_soal=tbl_banksoalpg.id_soal AND tbl_soaldetail.id_soalujian='$idsoalujian'");
