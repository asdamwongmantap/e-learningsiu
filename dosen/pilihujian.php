...
from tbl_pengguna p, tbl_dosen d where p.username=d.nid AND p.username='$nid'");
  ...
  box-shadow: 1px 1px 2px rgba(0,0,0,.3);"><center>
			<div class="span6 checkout-list" style="border:0px solid black;margin-top:30px;margin-bottom:20px;">
            <ol class="rr">
              <li class="current">
               <h3 style="text-align:left;">Pilih Jenis Soal Yang Akan Diberikan</h3>
                <div class="row">
                      ...
                      <form action="" enctype="multipart/form-data" method="POST">
                       ...
                       <ul class="rr">
                        <table  style="border:0pt dashed #0000CC;float:left;width:50%;" border="0" >
                          <tr><td><select name='kdmk' id='s2_1' style='width: 50%;'>
								  <option value=0 selected>- Pilih Jenis Soal -</option>
								  <option>Pilihan Ganda</option>
								  <option>Essay</option>
								     <?php
					
					$jenissoal=$_POST['kdmk'];
					
					if ($jenissoal=='Pilihan Ganda')
					{
					echo "<script>window.location = 'media.php?hal=inputujianpg'</script>";
					}
					elseif ($jenissoal=='Essay')
					{
					echo "<script>window.location = 'media.php?hal=inputujian'</script>";
					}
					?>
								  </select>
								  </td></tr>
