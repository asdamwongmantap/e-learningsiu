...
from tbl_pengguna p, tbl_mahasiswa mhs where p.username=mhs.nim AND p.username='$nim'");
  ...
  box-shadow: 1px 1px 2px rgba(0,0,0,.3);"><center>
    ...
    <h3 style="margin-top:10px;">Daftar Materi</h3>
<table class="layout display responsive-table">
	...
	<?php
	$nim=$_SESSION['username'];
		$tp=mysql_query("SELECT tbl_materi.kd_materi,tbl_materi.kd_mk, tbl_mahasiswa.kd_kelas, tbl_matakuliah.nma_mk, tbl_materi.kd_kelas, tbl_materi.judul_materi, tbl_materi.deskripsi, tbl_materi.filename FROM tbl_materi, tbl_matakuliah, tbl_mahasiswa, tbl_pengguna where  tbl_materi.kd_mk=tbl_matakuliah.kd_mk AND tbl_materi.kd_kelas=tbl_mahasiswa.kd_kelas AND tbl_pengguna.username=tbl_mahasiswa.nim AND tbl_pengguna.username='$nim'");
	...
	?>
        ...
        <tr>
                                    ...
                                    <td><?php echo $r['judul_materi'] ?></td>
									<td><?php echo $r['filename']?></td>
                                    <td><a href="downloadmateri.php?id=<?php echo $r['kd_materi']?>">Download</a></td>
        </tr>
    <?php
	}
	?>
